	...
	public function wps_shortcode( $attributes ) {
		...
		if ( $que->have_posts() ) {
			while ( $que->have_posts() ) :
				...
				$output .= '<div class="wpsf-product"><div class="product-slider-overlay" style="position:absolute;width:100%;height:100%;background:rgba(255,255,255,1);z-index: 99999;display: flex;align-items: center;justify-content: center;flex-direction:column;">' . do_shortcode( '[add_to_cart id="' . get_the_ID() . '" show_price="false"]' ) . do_shortcode('[ti_wishlists_addtowishlist product_id="'. get_the_ID() .'"]').'</div>';
